...
if command -v gpg >/dev/null 2>&1; then
  if [ -n "$GPG_KEY_ID" ]; then
    gpg --batch --yes --pinentry-mode loopback --default-key "$GPG_KEY_ID" --output "dists/$DIST/Release.gpg" --detach-sign "dists/$DIST/Release"
    gpg --batch --yes --pinentry-mode loopback --default-key "$GPG_KEY_ID" --output "dists/$DIST/InRelease" --clearsign "dists/$DIST/Release"
    gpg --batch --yes --output "roboclaw-studio.gpg" --export "$GPG_KEY_ID"
  else
    gpg --batch --yes --pinentry-mode loopback --output "dists/$DIST/Release.gpg" --detach-sign "dists/$DIST/Release"
    gpg --batch --yes --pinentry-mode loopback --output "dists/$DIST/InRelease" --clearsign "dists/$DIST/Release"
  fi
else
